...
class AuraRouterAdapter implements Router
{
    ...
    {
        $this->router->get($target, $path, function($request, $response) use ($target) {
            $controller = new $target();
            $response->body()->write($controller->get());
            return $response;
        });
    }
